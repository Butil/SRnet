...
def SR_net_model(features, mode):
    _inputs = tf.cast(tf.transpose(features, [0, 3, 1, 2]), tf.float32, name='input')
    data_format = 'NCHW'
    ...
    with arg_scope([layers.conv2d], num_outputs=16,
        ...
        with tf.variable_scope('Layer12'): # 16*16
            ...
            bn=layers.batch_norm(conv2)
            avgp = layers.avg_pool2d(bn, kernel_size=[16,16], stride=[16,16])
